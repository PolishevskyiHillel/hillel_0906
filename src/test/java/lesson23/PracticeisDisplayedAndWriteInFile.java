...
public class PracticeisDisplayedAndWriteInFile {
    ...
    public void assertTest() throws IOException {
        ...
        WebElement inStockLabel = driver.findElement(By.xpath("//*[contains(@class, 'status-label ')]"));
        ...
        String productPageTitleText = productPageTitle.getAttribute("innerTest");

        String productPriceText = driver.findElement(By.xpath("//*[contains(@class, 'product-price__big')]"))
                .getAttribute("innerText");

        if (inStockLabel.isDisplayed() && inStockLabelColor.equals(EXPECTED_IN_STOCK_BTN_COLOR)) {
            FileWriter fileWriter = new FileWriter("TitleAndPrice.txt");
            fileWriter.write(productPriceText + " - " + productPriceText);
            fileWriter.close();
        }
    }
